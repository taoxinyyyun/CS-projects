...
class PuzzleGenerator implements PuzzleSource {
    ...
    static Sq findUniqueSuccessor(Model model, Sq start) {
        ...
        int y1 = 0;
        int connector = 0;
        PlaceList successors = start.successors();
        for (Place x : successors) {
            if (start.connectable(model.get(x.x, x.y))) {
                x1 = x.x;
                y1 = x.y;
                connector += 1;
            }
        }
        if (connector == 1 && start.sequenceNum() == 0) {
            return model.get(x1, y1);
        } else if (connector == 1 && start.sequenceNum() != 0) {
            if (model.get(x1, y1).sequenceNum() == 0) {
                return model.get(x1, y1);
            }
        } else if (connector != 1 && start.sequenceNum() != 0) {
            for (Place x : successors) {
                if (model.get(x.x, x.y).sequenceNum()
                        == start.sequenceNum() + 1) {
                    return model.get(x.x, x.y);
                }
            }
        }
        return null;
    ...
    static Sq findUniquePredecessor(Model model, Sq end) {
        ...
        int x1 = 0;
        int y1 = 0;
        PlaceList predecessors = end.predecessors();
        for (Place x : predecessors) {
            if (model.get(x.x, x.y).connectable(end)) {
                x1 = x.x;
                y1 = x.y;
                connector += 1;
            }
        ...
        if (connector == 1) {
            if (end.sequenceNum() == 0) {
                ...
                if (model.get(x1, y1).sequenceNum() == end.sequenceNum() - 1) {
                    return model.get(x1, y1);
                }
            }
        }
    ...
    private void makeSolutionUnique(Model model) {
        System.out.println("---- New call to makeSolutionUnique ----");
        model.restart();
        AddNum:
        while (true) {
            System.out.println("Extending model.");
            extendSimple(model);
            System.out.printf("Unconnected: %d\n", model.unconnected());
            ...
            for (Place p : unnumbered) {
                ...
                System.out.println("Not able to extend. Trying next Place.");
            }
