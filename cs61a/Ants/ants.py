...
class Bee(Insect):
    ...
    def action(self, colony):
        ...
        # BEGIN EC
        if self.scared==True and self.direction=='backwards':
...
class ThrowerAnt(Ant):
    ...
    def nearest_bee(self, beehive):
        ...
        while current_place != beehive:
            if self.min_range < count_range < self.max_range and current_place.bees:
                return random_or_none(current_place.bees)
            elif count_range >= self.max_range:
                return None
            count_range += 1
...
class FireAnt(Ant):
    ...
    def reduce_armor(self, amount):
        ...
        # BEGIN Problem 5
        "*** YOUR CODE HERE ***"
        copy_bees = iter(self.place.bees[:])
...
# END Problem 13
    ...
    def action(self, colony):
        ...
        if self.true_queen:
            ...
            pointer = self.place
            place_lst = []
            while not pointer.exit == None:
                place_lst.append(pointer.exit)
                pointer = pointer.exit

            ants = []
            for place in place_lst:
                if not place.ant == None:
                    if isinstance(place.ant,BodyguardAnt) and place.ant.contained_ant:
                        ants.append(place.ant.contained_ant)
