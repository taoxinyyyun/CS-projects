...
# initial identifying symbol (if, lambda, quote, ...). Its rest argument is
...
def do_define_form(expressions, env):
    ...
    if scheme_symbolp(target):
        ...
        # BEGIN PROBLEM 5
        "*** YOUR CODE HERE ***"
        body = scheme_eval(expressions.rest.first, env)
        ...
        # BEGIN PROBLEM 9
        "*** YOUR CODE HERE ***"
        new_expr = Pair('lambda', Pair(target.rest, expressions.rest))
...
def make_let_frame(bindings, env):
    ...
    val_list = Pair(scheme_eval(temp.first.rest.first, env), nil)
    helper = lambda par, val: Pair(val, par)
    temp = temp.rest
    while not temp == nil:
        check_form(temp.first, 2, 2)
        formal_list = helper(formal_list, temp.first.first)
        val_list = helper(val_list, scheme_eval(temp.first.rest.first, env))
        temp = temp.rest
...
def do_define_macro(expressions, env):
    ...
    if scheme_symbolp(target):
        ...
        # BEGIN PROBLEM 9
        "*** YOUR CODE HERE ***"
        body = MacroProcedure(target.rest, expressions.rest, env)
        env.define(target.first, body)
        return target.first
        # END PROBLEM 9
    else:
...
def do_quasiquote_form(expressions, env):
    ...
    def quasiquote_item(val, env, level):
        ...
        rest = quasiquote_item(val.rest, env, level)
